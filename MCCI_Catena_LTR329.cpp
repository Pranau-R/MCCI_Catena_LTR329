...
void cLTR329::reset()
    ...
    writeByte(cLTR239_ADDR_ALS_CONTROL, ctrl.raw);

    // Wait for resetting
    do
        {
        Serial.print(".");
        ctrl.raw = readByte(cLTR239_ADDR_ALS_CONTROL);
        }
...
void cLTR329::writeMeasRate(ALS_INT_Enum _intTime, ALS_MEAS_Enum _measRate)
    ...
    writeByte(cLTR239_ADDR_ALS_MEAS_RATE, mr.raw);
    }

ALS_MEAS_RATE_REG cLTR329::readMeasRate()
    {
    ALS_MEAS_RATE_REG mr; 
    mr.raw = readByte(cLTR239_ADDR_ALS_MEAS_RATE);
    return mr;
    }
...
ALS_PS_STATUS_REG cLTR329::readStatus()
    {
    ALS_PS_STATUS_REG status;
    status.raw = readByte(cLTR239_ADDR_ALS_STATUS);
    return status;
    }
...
uint8_t cLTR329::readManufacturerId()
    {
    uint8_t data = readByte(cLTR239_ADDR_MANUFAC_ID);
    return data;
    }

uint8_t cLTR329::readPartNumber()
    {
    uint8_t data = readByte(cLTR239_ADDR_PART_ID);
    return data >> 4;
    }
...
uint8_t cLTR329::readByte(uint8_t addr)
    ...
    do
        {
        Wire.beginTransmission(cLTR239_I2C_ADDRESS);
        Wire.write(addr);
        Wire.endTransmission(false); // Restart
        delay(10);
        rdDataCount = Wire.requestFrom(cLTR239_I2C_ADDRESS, 1);
        }
...
uint16_t cLTR329::readAlsData(uint8_t ch)
    ...
    if(ch == 0)
        {
        addr_lsb = cLTR239_ADDR_ALS_DATA_CH_0_0;
        addr_msb = cLTR239_ADDR_ALS_DATA_CH_0_1;
        }
    else
        {
        addr_lsb = cLTR239_ADDR_ALS_DATA_CH_1_0;
        addr_msb = cLTR239_ADDR_ALS_DATA_CH_1_1;
        }
